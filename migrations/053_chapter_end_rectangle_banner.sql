...
-- Install the supplied chapter-end tag as its own isolated 300×250 unit and
-- enable the existing React-managed advertisement box after chapter content.
WITH upserted_unit AS (
  ...
  VALUES
    (
      ...
      'Adsterra Chapter End Rectangle 300×250',
      'Adsterra',
      'iframe',
      300,
      250,
$chapter_end_rectangle$<script>
(function(zkr){
var d = document,
...
l.parentNode.insertBefore(s, l);
  ...
  ON CONFLICT (id) DO UPDATE SET
    ...
    is_builtin = true,
    updated_at = timezone('utc'::text, now())
  WHERE (public.ad_placements.placement_key, public.ad_placements.page_type,
         public.ad_placements.slot, public.ad_placements.ad_unit_id,
         public.ad_placements.enabled, public.ad_placements.desktop_enabled,
         public.ad_placements.mobile_enabled, public.ad_placements.sort_order,
         public.ad_placements.default_slot, public.ad_placements.custom_settings,
...
UPDATE public.ad_settings
 ...
 WHERE id = 'global'
   AND (
     ...
     OR EXISTS (SELECT 1 FROM upserted_placement)
   );
