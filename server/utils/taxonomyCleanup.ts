...
export const SCRUB_BANNED_TAXONOMY_SQL = `
...
export async function scrubBannedTaxonomyFromDatabase() {
  if (!cleanupPromise) {
    cleanupPromise = db.query(SCRUB_BANNED_TAXONOMY_SQL)
      .then(() => undefined)
      .catch((error) => {
        cleanupPromise = null;
        const message = String(error?.code || error?.message || error || "");
        const unavailable = /ECONNREFUSED|ENOTFOUND|ETIMEDOUT|connection/i.test(message);
        console.warn(unavailable ? "Banned taxonomy cleanup skipped: database unavailable." : `Banned taxonomy cleanup skipped: ${message}`);
      });
  }
